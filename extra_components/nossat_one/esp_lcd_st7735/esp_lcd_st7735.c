...
static const st7735_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    // Software Reset
    {LCD_CMD_SWRESET, (uint8_t[]){}, 0, 150},
    // Sleep Out
    {LCD_CMD_SLPOUT, (uint8_t[]){}, 0, 255},
    ...
    {0xC2, (uint8_t[]){0x0A, 0x00}, 2, 0},
    // Power Control 4 (in Idle mode/ 8-colors)
    {0xC3, (uint8_t[]){0x8A, 0x2A}, 2, 0},
    // Power Control 5 (in Partial mode/ full-colors)
    {0xC4, (uint8_t[]){0x8A, 0xEE}, 2, 0},
    // VCOM Control 1
    {0xC5, (uint8_t[]){0x0E}, 1, 0},
    // Display Inversion Off
    {LCD_CMD_INVOFF, (uint8_t[]){}, 0, 0},
    // Memory Data Access Control - BGR color filter panel
    // {LCD_CMD_MADCTL, (uint8_t[]){0xC0}, 1, 0},
    // Interface Pixel Format - 16-bit/pixel 65K-Colors(RGB 5-6-5-bit Input)
    // {LCD_CMD_COLMOD, (uint8_t[]){0x05}, 1, 0},
    // Column Address Set
    {LCD_CMD_CASET, (uint8_t[]){0x00, 0x02, 0x00, 0x81}, 4, 0},
    // Row Address Set
    {LCD_CMD_RASET, (uint8_t[]){0x00, 0x01, 0x00, 0xA0}, 4, 0},
