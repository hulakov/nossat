...
{
    const i2s_slot_mode_t slot_mode = static_cast<i2s_slot_mode_t>(audio.num_channels);
    const i2s_data_bit_width_t data_bit_width = static_cast<i2s_data_bit_width_t>(audio.bits_per_sample);
    ...
    const i2s_std_config_t std_cfg = {
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(audio.sample_rate),
    ...
    ESP_ERROR_CHECK(i2s_channel_write(tx_handle, audio.data.data(), audio.data.size(), &bytes_written, 1000));
